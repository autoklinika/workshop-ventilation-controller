...
Rozłączyć testowy mostek pomiędzy DFR0845 i podłączyć `/dev/ttyAMA0` do rzeczywistej magistrali dwóch węzłów KAmod + SEN55.

Najpierw wykonać ręczny odczyt slave `1` i `2` istniejącym narzędziem diagnostycznym. Produkcyjnego `sensor_bus_worker` nie uruchamiać przed zakończeniem tej walidacji magistrali na CM5.

Nie wykonano merge i nie zmieniono statusu żadnego PR.
