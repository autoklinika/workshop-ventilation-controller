...
class GP8403:
    """Production adapter for the validated DFRobot DFR0971 / GP8403 DAC."""
        ...
        *,
        ...
        if bus_instance is not None:
            ...
            try:
                ...
                raise GP8403Error(
                    "Python SMBus module is missing. Install: sudo apt install python3-smbus"
                ) from exc
            try:
    ...
    def probe(self) -> int:
        try:
            ...
            raise GP8403Error(
                f"No response from GP8403 at 0x{self.config.address:02X}: {exc}"
            ) from exc
    ...
    def voltage_to_word(self, voltage: float) -> int:
        if not 0.0 <= voltage <= self.config.output_range_volts:
            raise ValueError(
                f"Voltage must be between 0.0 and {self.config.output_range_volts:.1f} V"
            )
        code = round((voltage / self.config.output_range_volts) * self.MAX_CODE)
        return min(max(code, 0), self.MAX_CODE) << 4

    def set_voltage(self, channel: int, voltage: float) -> None:
        if channel not in self.CHANNEL_REGISTERS:
            raise ValueError("Channel must be 0 or 1")
        self._write_word(self.CHANNEL_REGISTERS[channel], self.voltage_to_word(voltage))

    def set_both_channels(self, supply_voltage: float, extract_voltage: float) -> None:
        self.set_voltage(0, supply_voltage)
        self.set_voltage(1, extract_voltage)

    def zero_all(self) -> None:
        self.set_both_channels(0.0, 0.0)
    ...
    def _write_word(self, register: int, value: int) -> None:
        try:
            ...
            raise GP8403Error(
                f"I2C write failed at 0x{self.config.address:02X}, register 0x{register:02X}: {exc}"
            ) from exc
