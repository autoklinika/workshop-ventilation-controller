...
class TachoRuntimeTests(unittest.TestCase):
    def test_core_state_exposes_extract_tacho_read_only(self) -> None:
        ...
        self.assertEqual(actuator.applied, FanSetpoints(0.0, 5.0))

        payload = state.to_dict()
        self.assertEqual(payload["tacho"]["extract"]["rpm"], 1400.0)
        self.assertIsNone(payload["tacho"]["supply"])
    ...
    def test_tacho_runtime_is_opt_in_and_defaults_to_validated_gpio27(self) -> None:
        parser = build_parser()
        ...
        enabled_args = parser.parse_args(["--enable-extract-tacho"])
