...
WT=/home/wentylacja/wvc-control-engine-stage7a-tacho-validation
...
CONFIRM_PHYSICAL_SPIN="${CONTROL_ENGINE_STAGE7A_CONFIRM_PHYSICAL_FAN_SPIN:-}"
CORE_UNIT=ventilation-core.service
CORE_DROPIN_DIR=/etc/systemd/system/${CORE_UNIT}.d
CORE_DROPIN=${CORE_DROPIN_DIR}/99-zz-control-engine-stage7a-tacho-validation.conf
...
[ "$CONFIRM_PHYSICAL_SPIN" = "YES" ] || fail "set CONTROL_ENGINE_STAGE7A_CONFIRM_PHYSICAL_FAN_SPIN=YES to acknowledge real fan motion"
[ -n "$EXPECTED_BRANCH_SHA" ] || fail "set CONTROL_ENGINE_STAGE7A_EXPECTED_BRANCH_SHA to exact CI-tested branch SHA"

cd "$ROOT"
echo "===== CONTROL ENGINE STAGE7A CM5 TACHO SPIN-UP VALIDATION ====="
echo "WARNING: this test intentionally commands both local EC fans to 2.0 V, then returns to STOP / 0 V"
...
echo "===== 3. PHYSICAL 0 -> 2 V -> 0 V TACHO MEASUREMENT ====="
env PYTHONDONTWRITEBYTECODE=1 PYTHONPATH="$WT/src" \
    /usr/bin/python3 -B "$WT/tools/validate_control_engine_stage7a_tacho_spinup_cm5.py" \
...
require_safe_start "$WT/src" "after Stage7A measurement"
assert_host_untouched "after Stage7A measurement"

echo "===== 4. RESULT ====="
echo "PASS: Stage7A physical TACHO spin-up measurement completed"
