...
WT=/home/wentylacja/wvc-control-engine-stage7c-tacho-validation
...
CONFIRM_PHYSICAL_SPIN="${CONTROL_ENGINE_STAGE7C_CONFIRM_PHYSICAL_FAN_SPIN:-}"
CORE_UNIT=ventilation-core.service
CORE_DROPIN_DIR=/etc/systemd/system/${CORE_UNIT}.d
CORE_DROPIN=${CORE_DROPIN_DIR}/99-zz-control-engine-stage7c-tacho-validation.conf
...
[ "$CONFIRM_PHYSICAL_SPIN" = "YES" ] || fail "set CONTROL_ENGINE_STAGE7C_CONFIRM_PHYSICAL_FAN_SPIN=YES to acknowledge real fan motion"
[ -n "$EXPECTED_BRANCH_SHA" ] || fail "set CONTROL_ENGINE_STAGE7C_EXPECTED_BRANCH_SHA to exact CI-tested branch SHA"

cd "$ROOT"
echo "===== CONTROL ENGINE STAGE7C CM5 1.0 V TACHO LOW-SPEED CHARACTERIZATION ====="
echo "WARNING: this test intentionally runs both local EC fans at 1.0 V for 20 s x 3 cycles, with STOP / 0 V between cycles"
...
echo "===== 3. PHYSICAL 3-CYCLE 1.0 V LOW-SPEED CHARACTERIZATION ====="
...
require_safe_start "$WT/src" "after Stage7C characterization"
assert_host_untouched "after Stage7C characterization"

echo "===== 4. RESULT ====="
echo "PASS: Stage7C extended physical 1.0 V TACHO characterization completed"
echo "PASS: all three cycles used fixed 20 s hold at 1.0 V"
