...
WT=/home/wentylacja/wvc-safe-shutdown-active-validation
...
echo "===== SAFE SHUTDOWN ACTIVE -> OFF VALIDATION ====="
cd "$ROOT"
...
if result.get("physical_confirmation") is not True:
    raise SystemExit(f"FAIL: final AERO speed 0 lacks physical confirmation: {result!r}")
observed = result.get("observed_power") or {}
